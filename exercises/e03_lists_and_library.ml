...
let rec product = function [] -> 1 | front :: rest -> front * product rest
let rec concat = function [] -> "" | str :: rest -> str ^ concat rest
...
let starts_with_bigred = function "big" :: "red" :: _ -> true | _ -> false
let length_is_two_or_four = function [ _; _ ] | [ _; _; _; _ ] -> true | _ -> false
...
let fifth_or_zero xs = match List.nth_opt xs 4 with Some fifth -> fifth | None -> 0
...
let descending xs = List.sort (fun a b -> Int.compare b a) xs
let ascending xs = List.sort Int.compare xs
...
let any_zero xs = List.exists (fun c -> c = 0) xs
...
let () = print_endline "all tests passed! :D"
