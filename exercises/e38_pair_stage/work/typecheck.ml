...
let rec typeof environment = function
  ...
  | Let (name, bound, body) ->
      let bound_type = typeof environment bound in
      typeof ((name, bound_type) :: environment) body
  ...
  | If (condition, yes_branch, no_branch) ->
      ...
      let yes_type = typeof environment yes_branch in
      if yes_type = typeof environment no_branch then yes_type
